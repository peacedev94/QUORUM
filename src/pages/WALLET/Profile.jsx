...
const Profile = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A12] text-white relative px-3 pt-10 pb-28">
      {/* Profile Section */}
      <div className="absolute top-10 left-6 text-[#D1BCFF] cursor-pointer" onClick={() => navigate("/wallet")}>
        <BackArrowIcon />
      </div>
      <div className="flex flex-col pt-14 items-center mb-8">
        <img
          src={profile}
          alt="Profile"
      ...
      <div className="space-y-4">
        {menuItems.map((item, index) => (
          <button
            key={index}
            onClick={item.action}
            className="w-full border border-[#7B3FF2] rounded-lg px-6 py-4 flex items-center justify-between"
          >
