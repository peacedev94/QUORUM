...
const TestDash = () => {
  ...
  return (
    <div className="min-h-screen bg-[#090A10] text-white flex flex-col px-5 pt-12 pb-10 antialiased select-none">
      ...
      {/* Greeting Title */}
      <div className="mb-10">
        <h1 className="text-[26px] font-medium tracking-tight text-gray-100">Welcome, Organizer</h1>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-16">
        {stats.map((stat, index) => (
          <div 
            key={index} 
            className="bg-[#181931]/40 border-2 border-[#2E305E]/50 rounded-xl py-7 flex flex-col items-center justify-center text-center"
          >
            <p className="text-sm text-gray-300 font-normal tracking-wide mb-2">
              {stat.label}
            </p>
      ...
      <div className="mb-8">
        <h2 className="text-2xl font-normal tracking-tight text-gray-100">Quick Access</h2>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2 w-full">
        {quickAccessItems.map((item, index) => (
          ...
          >
            ...
            <div className="w-[72px] h-[72px] bg-[#7F4DFF] hover:bg-[#6A3BE6] active:scale-[0.96] transition-all rounded-xl flex items-center justify-center text-white shadow-lg mb-3">
              {item.icon}
            </div>
            {/* White labeling positioned perfectly beneath */}
            <span className="text-[12px] font-normal text-white text-center tracking-tight leading-tight whitespace-nowrap">
              {item.label}
            </span>
          </div>
        ))}
